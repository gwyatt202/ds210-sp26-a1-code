use kalosm::language::*;
use crate::solution::file_library;


pub struct ChatbotV4 {
    model: Llama,
}

impl ChatbotV4 {
    pub fn new(model: Llama) -> ChatbotV4 {
        return ChatbotV4 {
            model: model,
        };
    }

   


    //student 1 (george)
    pub async fn chat_with_user(&mut self, username: String, message: String) -> String {
        let filename = &format!("{}.txt", username);


        let mut chat_session: Chat<Llama> = match file_library::load_chat_session_from_file(filename) {
            //tries to load the session stored in the file and checks if some or none
            Some(old_session) => { self.model.chat().with_session(old_session)
            }, //the session found is initialised as old_session, and we start a new chat but load old_session
            None => { self.model.chat().with_system_prompt("The assistant will act like a old man")
            // new chat is initialised and system prompt is needed cuz no prior history

            }

        };

        let asynchronous_output = chat_session.add_message(message); 
        //specify which object in the struct to call to .add_message
        let output = asynchronous_output.await;

        file_library::save_chat_session_to_file(filename, &chat_session.session().unwrap());

        return output.unwrap();

        // TODO: You have to implement the rest:
        // You need to load the chat session from the file using file_library::load_chat_session_from_file(...).
        // Think about what needs to happen if the function returns None vs Some(session).
        // Hint: look at https://docs.rs/kalosm/latest/kalosm/language/struct.Chat.html#method.with_session

        return String::from("Hello, I am not a bot (yet)!");
    }


    //student 2 (phil)
    pub fn get_history(&self, username: String) -> Vec<String> {
    let filename = &format!("{}.txt", username);
    
    match file_library::load_chat_session_from_file(&filename) {
        None => {
            return Vec::new();
        }
        Some(session) => {
            // TODO: what should happen here?
            return Vec::new();
        }
    }
}
}