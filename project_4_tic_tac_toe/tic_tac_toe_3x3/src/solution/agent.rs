...
impl Agent for SolutionAgent {
    ...
    fn solve(board: &mut Board, player: Player, _time_limit: u64) -> (i32, usize, usize) {
        ...
        // `SolutionAgent::solve(...)`
        let next_move = minimax(board, player);
        return next_move; //returning the move that the minimax algorithm generated
    }
}
...
fn minimax(board: &mut Board, player: Player) -> (i32, usize, usize) {  
    ...
    for coord in avail_moves {
        ...
        match player {
                Player::X => {             //if we play as X we are maximising (equality is score > best_score)
                    if score > best_score { //if the score in the in the iteration is bigger than -inf, 
                        best_coord = coord; // replace the placeholder coord with the iterator coord
                        best_score = score; //replace the best_score with the score that fulfilled the condition
                    }
                }
                Player::O => { //if we play as 0 we are minimising (equality is score < best_score)
